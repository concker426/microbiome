...
Inflammatory bowel disease (IBD) is a chronic, relapsing inflammatory condition affecting millions worldwide, and gut microbiome dysbiosis is a consistent hallmark of the disease~\citep{paidimarri2024}. Stool metagenomic profiling thus offers a non-invasive route to diagnosis --- provided a model can both classify accurately and justify each prediction with verifiable evidence. We address this task: given a genus-level relative-abundance profile, predict IBD and provide the evidence behind the prediction. Three challenges dominate. \textbf{First, data scarcity}: only thousands of labeled samples support the microbiome$\rightarrow$IBD mapping. \textbf{Second, study variability (batch effects)}: cohort differences in laboratory, protocol, and platform often exceed biological signal. \textbf{Third, class imbalance}: public cohorts over-represent disease cases, so naive training inherits cohort-specific structure. A fourth requirement --- inspectable evidence --- distinguishes this task from pure classification.

Existing methods fall short along two dimensions. Classical classifiers (random forests, gradient boosting, logistic regression) discriminate well but offer no learned representation space for attribution or retrieval. Microbiome foundation models (MGM, BiomeGPT, Waypoint) do not transfer --- MGM reaches 50.9\% accuracy, below a majority-class baseline --- and interpret via post-hoc attention rather than verifiable evidence.

We therefore propose SimpleEmb, a minimal embedding classifier designed from the task rather than a foundation-model blueprint: each present genus is mapped to a learned embedding, masked mean pooling summarizes the profile, and a small MLP predicts IBD; the same embedding space powers leave-one-out (LOO) attribution, verbalized by a frozen Qwen2-7B. On clean\_2538, SimpleEmb reaches 92.57\% accuracy (AUC 0.975) with 1.1M parameters and no pretraining --- comparable to the strongest tree ensembles and 41.7 points above the 34M-parameter MGM encoder --- with LOO attribution (Spearman $\rho{=}0.72$) and explanations that halve hallucination (0.72 to 0.36 per response). Our contributions are as follows.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.7\textwidth]{figures/dataset_architecture_figure.png}
...
\textbf{Grouped-CV findings}: HistGradientBoosting (0.925) is numerically ahead of SimpleEmb+MLP (0.914) by 1.1pp --- no superiority over boosted trees is claimed. SimpleEmb+MLP significantly exceeds every MGM variant by 6.1--9.9pp and outperforms the un-embedded MLP (0.872) by 4.2pp, while providing the embedding space tree models lack (kNN, LOO attribution, clustering, LLM grounding). FDR-corrected McNemar tests (Table~\ref{tab:mcnemar}, Appendix) show SimpleEmb+MLP significantly beats KNN ($q{=}0.000$) and BernoulliNB ($q{=}0.015$), while Random Forest significantly beats SimpleEmb+MLP ($q{=}0.006$). A genus-level BiomeGPT architecture transfer (8-layer Transformer, 14.6M params, pretrained and fine-tuned from scratch per fold) is numerically the strongest here (ACC 0.937 / AUROC 0.981) but statistically indistinguishable from HistGradientBoosting (paired Wilcoxon, $p{=}0.229$), costs 13$\times$ the parameters of SimpleEmb plus per-fold pretraining, and yields no LOO attribution --- so it does not alter our interpretability conclusion. SIAMCAT (AUROC 0.975) and DeepMicro (AUROC 0.917) complete the external-baseline comparison.
...
We also test whether Qwen2-7B-Instruct can diagnose IBD directly via in-context learning. With up to five (genus list, diagnosis) examples, the LLM reaches only 62.0\% accuracy versus 92.0\% for the trained classifier on identical samples, with no reliable scaling ($k{=}1$: 59.6\%, $k{=}3$: 55.0\%; Table~\ref{tab:fewshot}, Appendix). This reinforces the separation of concerns: LLMs are effective interpreters of LOO-grounded evidence, not reliable predictors from raw microbiome profiles.
...
On the holdout (5 seeds; Table~\ref{tab:map_ablation}), only token dropout beats the minimal SimpleEmb (\textbf{93.89\%/0.9822} vs.\ 92.46\%/0.9753); gated pooling, a linear head, and masked pretraining all degrade. Under the 15-fold retention rule (Table~\ref{tab:map_retention}), token dropout wins 15/15 folds but is CI-significant in only 8/15 --- short of the 10/15 bar, with external transfer untested. No module is formally retained: SimpleEmb stays minimal and token dropout is the sole follow-up.
...
SimpleEmb shows that effective microbiome-based diagnosis needs neither large Transformer encoders, massive pretraining, nor LLM-based prediction: a 1.1M-parameter randomly-initialized embedding with masked mean pooling reaches 92.57\% accuracy on IBD, statistically comparable to strong tree ensembles and 41.7pp above a 34M pretrained MGM Transformer, with reliable LOO attribution (Spearman $\rho{=}0.72$) and LOO-grounded explanations that achieve 98\% consistency, 0.94 direction accuracy, and halved hallucination.
...
\begin{table}[ht]
\centering
\caption{\textbf{Few-shot LLM evaluation on IBD diagnosis ($n{=}100$).} Qwen2-7B-Instruct is given $k$ examples of (genus list, diagnosis) pairs and asked to predict on new test samples. The specialized classifier (SimpleEmb+MLP) outperforms the LLM by a large margin.}
\label{tab:fewshot}
\begin{tabular}{lcccc}
\toprule
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/dataset_statistics_figure.png}
...
Cross-class cosine similarity & 0.56 \\
Functional-group intra-group cosine & 0.0005 (vs.\ 0.0046 random) \\
...
\end{tabular}
\end{table}
% --- Supplementary data tables for experiments referenced in the main text ---
\begin{table}[ht]
\centering
\caption{\textbf{Pairwise McNemar tests (SimpleEmb vs.\ baselines on grouped-CV predictions).} $n_\text{disc}$ = discordant pairs; $B$ / $C$ = pairs where only SimpleEmb / only the baseline is correct. $q_\text{BH}$ = Benjamini--Hochberg FDR-corrected $p$-value. Bold = significant at $q<0.05$.}
\label{tab:mcnemar}
\begin{tabular}{lcccc}
\toprule
\textbf{Baseline} & $n_\text{disc}$ & $B$ & $C$ & $q_\text{BH}$ \\
\midrule
KNN & 329 & 213 & 116 & \textbf{$1.2{\times}10^{-6}$} \\
Random Forest & 151 & 55 & 96 & \textbf{$6.4{\times}10^{-3}$} \\
BernoulliNB & 375 & 216 & 159 & \textbf{$1.5{\times}10^{-2}$} \\
MGM + Logistic Regression & 286 & 161 & 125 & $1.1{\times}10^{-1}$ \\
MGM + Random Forest & 406 & 186 & 220 & $2.4{\times}10^{-1}$ \\
Logistic Regression & 197 & 88 & 109 & $2.8{\times}10^{-1}$ \\
RBF-SVM & 228 & 125 & 103 & $2.8{\times}10^{-1}$ \\
HistGradientBoosting & 139 & 75 & 64 & $5.9{\times}10^{-1}$ \\
Linear SVM & 278 & 133 & 145 & $6.8{\times}10^{-1}$ \\
MGM + MLP & 299 & 154 & 145 & $7.7{\times}10^{-1}$ \\
MLP & 235 & 115 & 120 & $8.7{\times}10^{-1}$ \\
Extra Trees & 156 & 79 & 77 & $9.4{\times}10^{-1}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[ht]
\centering
\caption{\textbf{Calibration metrics and error analysis on the held-out test set ($n{=}167$).}}
...
Specificity & 0.9324 \\
\midrule
\multicolumn{2}{l}{\textit{Error breakdown ($12/167 = 7.2\%$)}} \\
False positives (Healthy$\rightarrow$IBD) & 5 (conf.\ 0.71--0.86) \\
False negatives (IBD$\rightarrow$Healthy) & 7 (conf.\ 0.007--0.056) \\
\bottomrule
\end{tabular}
\end{table}
