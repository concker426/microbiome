...
Low Jaccard (0.07) but high Spearman $\rho$ (0.72) reflects functional substitution, and deletion tests confirm genuine signal.

\subsection{Module-Transfer Ablation: What Survives?}

Executed on the clean-2538 holdout (5 seeds, identical protocol; Table~\ref{tab:map_ablation}): A0 baseline 92.46$\pm$0.72\% ACC / 0.9753$\pm$0.005 AUROC; A1 +rank-bin (MGM) \textbf{93.77$\pm$0.29\%} / 0.9774$\pm$0.003; A2 +abundance-bin 92.93$\pm$1.16\% / 0.9749$\pm$0.005 (no gain); A4 +masked-abundance pretraining 91.26$\pm$0.81\% / 0.9734$\pm$0.004 (degrades; loss plateaus early). Only rank-bin survives the retention rule; masked pretraining is rejected and the name SimpleEmb-MAP is retired --- at 659-sample scale, foundation-model modules do not beat the minimalist embedding.
