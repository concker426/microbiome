...
\textbf{Grouped-CV findings}: HistGradientBoosting (0.925) is numerically ahead of SimpleEmb+MLP (0.914) by 1.1pp --- no superiority over boosted trees is claimed. SimpleEmb+MLP significantly exceeds every MGM variant by 6.1--9.9pp and outperforms the un-embedded MLP (0.872) by 4.2pp, while providing the embedding space tree models lack (kNN, LOO attribution, clustering, LLM grounding). FDR-corrected McNemar tests show SimpleEmb+MLP significantly beats KNN ($q{=}0.000$) and BernoulliNB ($q{=}0.015$), while Random Forest significantly beats SimpleEmb+MLP ($q{=}0.006$). A genus-level BiomeGPT architecture transfer (8-layer Transformer, 14.6M params, pretrained and fine-tuned from scratch per fold) is numerically the strongest here (ACC 0.937 / AUROC 0.981) but statistically indistinguishable from HistGradientBoosting (paired Wilcoxon, $p{=}0.229$), costs 13$\times$ the parameters of SimpleEmb plus per-fold pretraining, and yields no LOO attribution --- so it does not alter our interpretability conclusion. SIAMCAT (AUROC 0.975) and DeepMicro (AUROC 0.917) complete the external-baseline comparison.
