...
Table~\ref{tab:llm} summarizes the 167-sample evaluation of three prompt variants (Raw, LOO, LOO+Lit). LOO grounding halves hallucination (0.72 to 0.36 genera/response), raises prediction consistency from 63\% to 98\%, and improves direction accuracy from 0.27 to 0.94; adding literature context provides no further gain, consistent with the model discovering dataset-specific patterns beyond canonical markers. Four representative cases (correct, borderline, misclassified, extreme-IBD) illustrate that explanations mirror model confidence and expose atypical microbiomes in false negatives.
