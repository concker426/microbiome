...
We executed the first stage of this program on the clean-2538 held-out split (659 train / 167 test) under the identical 5-seed protocol as the main model: A0 (SimpleEmb baseline), A1 (+MGM-style rank-bin embedding, 32 bins), A2 (+abundance-bin embedding, within-sample quantile, 32 bins), and A4 (A2 + BiomeGPT-style masked-abundance pretraining: 25\% token masking, 30 pretraining epochs).
...
\midrule
A0: SimpleEmb (baseline) & 0.9246$\pm$0.0072 & 0.9753$\pm$0.0046 & \textbf{0.9829$\pm$0.0028} \\
A1: +rank-bin (MGM) & \textbf{0.9377$\pm$0.0029} & \textbf{0.9774$\pm$0.0029} & 0.9826$\pm$0.0027 \\
...
Only the MGM-style rank-bin encoding survives: accuracy rises from 92.46\% to 93.77\% (non-overlapping $\pm$1$\sigma$ bands, seed variance halved), and it is the sole module that degrades no metric. Abundance-bin adds nothing beyond rank, and masked-abundance pretraining actively hurts (91.26\%; the pretraining loss plateaus at 2.46 within 30 epochs) --- with 659 training samples, the auxiliary reconstruction objective does not transfer to classification. Under the pre-defined retention rule, the masked-pretraining branch is rejected, the name \emph{SimpleEmb-MAP} is retired, and the minimalist SimpleEmb --- optionally with rank-bin, pending the full 15-fold CI test --- remains the recommended backbone. This negative result reinforces the central claim: at current cohort scales, architectural modules imported from the foundation-model literature do not outperform a well-regularized minimal embedding.
