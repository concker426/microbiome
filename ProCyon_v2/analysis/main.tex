...
\subsection{Planned Extensions (SimpleEmb-MAP)}

We outline a minimal ablation program for augmenting SimpleEmb while preserving interpretability: (A0) the current SimpleEmb + mean-pool + MLP; (A1) rank-bin embedding (MGM); (A2) abundance-bin embedding; (A3) gated pooling; (A4) masked-abundance pretraining (BiomeGPT); (A5) A3+A4; (A6) token dropout (DeepMicro); (A7) taxonomy regularization (MDeep); and (A8) a linear head to isolate the role of the MLP. A module is retained only if it improves AUROC/AUPRC with non-overlapping paired-bootstrap 95\% CIs in at least 10 of 15 folds, does not degrade AUPRC, and transfers to at least one low-data or external setting. We provisionally name the final augmented model SimpleEmb-MAP (masked abundance pretraining), contingent on the pretraining ablation outperforming the no-pretraining backbone.
...
\bibitem{mdeep2021}
Wang, et al.\ \textit{MDeep: a deep learning approach for disease prediction based on microbiome data.}\
2021.
