...
\begin{abstract}
We present ProCyon v2, a microbiome analysis framework that decouples disease classification from biological interpretation. Through systematic architecture ablation on the clean\_2538 IBD dataset (659 train, 167 test), we find that: (1) a minimal encoder—nn.Embedding with masked mean pooling, no pretraining, no Transformer—achieves 92.57\% IBD diagnosis accuracy (AUC=0.975), outperforming a 34M-parameter pretrained MGM Transformer encoder by 41.7 percentage points (92.57\% vs 50.9\%); (2) XGBoost on raw genus abundance is a strong baseline (92.22\%), but lacks the embedding structure needed for interpretability; (3) embedding dimension saturates at $E{=}512$ (93.41\%), requiring only 760K parameters; (4) cross-cohort validation shows training on $5\times$ more diverse data improves generalization from 61.8\% to 88.6\% ACC; (5) SHAP Spearman rank correlation $\rho{=}0.72$ across folds confirms consistent genus importance ranking, with deletion tests showing 5.6$\times$ larger AUC drop than random; (6) Qwen2-7B-Instruct fails as a classifier (70.7\% $\pm$ 11.9\% instability) but excels as an interpreter when grounded by SHAP—achieving zero hallucination rate and 98\% prediction consistency. ProCyon v2 establishes that the model discovers patterns, SHAP provides evidence, and the LLM explains the evidence.
\end{abstract}
...
\begin{itemize}
  ...
  \item \textbf{Cross-cohort validation}: Training on $5\times$ larger diverse data improves cross-cohort generalization by 26.8pp
  \item \textbf{SHAP reliability}: Spearman $\rho{=}0.72$ rank consistency, deletion test validation, literature direction agreement (5/6)
  \item \textbf{LLM explanation benchmark}: SHAP grounding eliminates hallucination (0.0 vs 0.3/response), achieves 98\% consistency
\end{itemize}
...
MLP($768{\rightarrow}256{\rightarrow}\text{BN}{\rightarrow}\text{ReLU}{\rightarrow}\text{Drop}{\rightarrow}2$) \quad SHAP LOO Importance \\
...
\subsection{Cross-Cohort Validation}

\begin{table}[ht]
\centering
\caption{\textbf{Cross-cohort validation.} ProCyon v2 (SimpleEmb+MLP, end-to-end)
trained on one cohort and evaluated on another. merged\_all provides $5\times$ more training data.}
\label{tab:cross}
\begin{tabular}{lccccc}
\toprule
\textbf{Train $\rightarrow$ Test} & \textbf{$n_\text{train}$} & \textbf{ACC} & \textbf{AUC} & \textbf{Rec.} & \textbf{Spec.} \\
\midrule
...
\textbf{Finding}: merged$\rightarrow$clean retains 94.9\% of within-cohort performance (0.8862/0.9341), while clean$\rightarrow$merged retains only 77.2\%. Training on $5\times$ more diverse data substantially improves generalization. The clean$\rightarrow$merged failure mode (specificity collapse: 100\%, sensitivity: 25.4\%) indicates that small-dataset models become overly conservative when facing out-of-distribution samples.
...
The comparable performance of DeepSets and ProCyon v2 suggests they share the same permutation-invariant inductive bias. However, SimpleEmb additionally provides an explicit genus embedding space (unlike DeepSets' implicit element encoder) that enables: (a) SHAP feature attribution for per-sample genus importance, (b) kNN retrieval and clustering for patient stratification (kNN ACC: 97.0\%), (c) cosine similarity analysis for embedding quality assessment, and (d) LLM-grounded explanation through SHAP-to-text conversion. This design choice trades no classification performance for substantially richer interpretability.
...
\subsection{SHAP Reliability}

\begin{table}[ht]
\centering
\caption{\textbf{SHAP reliability analysis across 5 folds.}}
\label{tab:shap}
\begin{tabular}{lc}
\toprule
\textbf{Metric} & \textbf{Value} \\
...
The deletion test confirms biological relevance: removing the top-50 SHAP-identified genera reduces AUC by 0.039, 5.6$\times$ more than random deletion (0.007). SHAP importance is uncorrelated with genus prevalence ($r{=}0.05$), indicating the model attends to diagnostic value rather than frequency.
...
Three prompt variants evaluated: Raw (genus list only), SHAP (classifier output + SHAP attributions),
SHAP+Lit (SHAP + literature context). Bold indicates best.}
\label{tab:llm}
\begin{tabular}{lcccc}
\toprule
...
SHAP grounding eliminates hallucination entirely (0.0 vs 0.3 genera/response) and achieves 98\% prediction consistency—the LLM's explanation aligns with the classifier's output. Adding literature context (variant C) slightly reduces performance (94\% consistency, 0.68 specificity), likely because the model discovers non-canonical markers that literature annotations may contradict.

\textbf{Case Study}: Patient 2538.1003527 (IBD, $p{=}1.000$). Top SHAP genera: Caldicellulosiruptor $\uparrow$, Desulfitobacterium $\uparrow$, Defluviitoga $\downarrow$, Thermoflavimicrobium $\downarrow$, Chryseobacterium $\downarrow$. SHAP-grounded LLM: ``The classifier predicted IBD based on the enrichment of genera associated with gut inflammation (Caldicellulosiruptor, Desulfitobacterium) and depletion of genera typically found in healthy gut microbiomes (Defluviitoga, Thermoflavimicrobium). This pattern is consistent with the dysbiosis observed in IBD patients, where pro-inflammatory genera proliferate at the expense of commensal species.''
...
ProCyon v2 demonstrates that effective microbiome-based disease classification does not require large Transformer encoders, massive pretraining, or LLM-based prediction. A 1.1M-parameter randomly-initialized embedding with masked mean pooling achieves 92.57\% accuracy on IBD diagnosis, exceeding a 34M pretrained MGM Transformer by 41.7pp. The embedding dimension saturates at $E{=}512$ (760K params), and cross-cohort validation confirms generalization (merged$\rightarrow$clean: 88.62\%). SHAP attributions are reliable (Spearman $\rho{=}0.72$, deletion test validated), and SHAP-grounded LLM explanations achieve zero hallucination with 98\% consistency.

The core insight is the \textbf{separation of concerns}: the model discovers patterns from data, SHAP provides evidence for those patterns, and the LLM explains the evidence in natural language. Each component is optimized for its specific role, and the pipeline is verifiable at every stage.
