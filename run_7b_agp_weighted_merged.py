#!/usr/bin/env python3
"""
7B AGP merged (existing + FTP) genus-level classification — class-weighted training.
Qwen2.5-7B-Instruct + LoRA, Healthy vs Disease
Uses expanded dataset for improved Disease recall.
"""
...
DATA_DIR = "/hd/liujx/microbiome_llm_project/data/agp_ftp_merged"
TRAIN_DATA = os.path.join(DATA_DIR, "train_set.jsonl")
TEST_DATA = os.path.join(DATA_DIR, "test_set.jsonl")
OUTPUT_DIR = "/hd/liujx/microbiome_llm_project/saved_models/qwen2.5_7b_agp_merged_weighted"
EVAL_DIR = "/hd/liujx/microbiome_llm_project/eval_results_7b_agp_merged_weighted"
...
# Training set: 11391 Healthy + 327 Disease → ratio ~35:1
CLASS_WEIGHTS = {"Healthy": 1.0, "Disease": 35.0}
...
def deduplicate_by_sample_id(data):
    """Remove duplicates by sample_id, keep first occurrence."""
    seen = set()
    deduped = []
    for item in data:
...
def main():
    print("=" * 60)
    print("  7B AGP Merged (existing+FTP) — 加权训练")
    print("  Qwen2.5-7B-Instruct + LoRA + Class-Weighted Loss")
    print("  Healthy vs Disease (扩增数据)")
    print("=" * 60)
    ...
    results_after = evaluate(eval_model, tokenizer, test_data, device,
                             name="微调后 (7B, merged, 类别加权)", max_new_tokens=64)

    # 6. Summary
    print(f"\n{'='*60}")
    print(f"  对比总结")
    print(f"{'='*60}")
    print(f"{'指标':<20} {'基座模型':<22} {'微调后':<22}")
    print(f"{'-'*20} {'-'*22} {'-'*22}")
    ...
    results = {
        ...
        "task": "agp_ftp_merged_binary_weighted",
