...
def main():
    ...
    llm = AutoModelForCausalLM.from_pretrained(MODEL_PATH, quantization_config=bnb_config, device_map={"": "cuda:0"}, trust_remote_code=True, local_files_only=True)
    ...
    micro_encoder = MicrobiomeEncoderV2(num_species=NUM_SPECIES, hidden_size=EMBED_DIM, num_layers=2, dropout=0.1).to("cuda:0")
    projection = ProjectionLayer(input_dim=EMBED_DIM, output_dim=LLM_HIDDEN_SIZE).to("cuda:0")
    multi_task_loss = MultiTaskLoss(clm_weight=1.0, contrastive_weight=0.3, temperature=0.07).to("cuda:0")
