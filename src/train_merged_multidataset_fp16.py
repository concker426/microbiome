"""
改进的训练脚本 - 使用FP16而非4bit量化（避免CUDA版本兼容问题）
"""
import os
import torch
import pandas as pd
...
EPOCHS = 5
BATCH_SIZE = 1  # FP16需要更小的batch size
...
def main():
    print("="*60)
    print("🚀 开始多数据集联合训练 (FP16 + Transformer Encoder)")
    print("="*60)
    ...
    # 2. 初始化LLM (使用FP16，不使用4bit量化)
    print("\n🤖 初始化LLM模型 (Qwen2.5-7B FP16 + LoRA)...")
    ...
    # 4. 投影层 (共享)
    projection = ProjectionLayer(input_dim=EMBED_DIM, output_dim=LLM_HIDDEN_SIZE).to("cuda:0").half()
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            otu_vectors = batch['otu_vector'].to("cuda:0").half()  # FP16
