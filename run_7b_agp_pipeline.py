...
OUTPUT_DIR = "/hd/liujx/microbiome_llm_project/saved_models/qwen2.5_7b_agp_genus"
EVAL_DIR = "/hd/liujx/microbiome_llm_project/eval_results_7b_agp"
...
def main():
    print("=" * 60)
    print("  7B AGP Genus-level 分类")
    print("  Qwen2.5-7B-Instruct + LoRA")
    ...
    results_after = evaluate(eval_model, tokenizer, test_data, device,
                             name="微调后 (7B, AGP genus, 平衡)", max_new_tokens=64)

    # 6. Summary
    print(f"\n{'='*60}")
    print(f"  对比总结")
    print(f"{'='*60}")
    print(f"{'指标':<20} {'基座模型(7B)':<22} {'LoRA微调后':<22}")
    print(f"{'-'*20} {'-'*22} {'-'*22}")
