...
OUT = ROOT / "experiments/results/decontaminated_groupcv_representation_ablation_20260804"
VOCAB_SIZE = 1226
EMBED_DIM = 64
HIDDEN_DIM = 256
EPOCHS = 50
...
def main() -> None:
    ...
    for seed in SEEDS:
        ...
        for fold, (train_index, valid_index) in enumerate(splitter.split(features, labels, groups), start=1):
            ...
            for model_number, (model_name, (_, model_type)) in enumerate(MODELS.items()):
                target, predicted, scores, params, runtime = train_fold(model_name, model_type, sequences, masks, features, labels, train_index, valid_index, seed * 10000 + fold * 10 + model_number)
                metric = {
