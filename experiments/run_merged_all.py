#!/usr/bin/env python3
"""A: merged_all + Adapter 8t × 3 seeds. Fast batched generate() eval."""
...
V=1226;E=768;LH=3584;SL=175;NT=8;PS=0.1;LR_R=16;LR_A=32;LR_D=0.03
...
# Proven autoregressive eval (same as H4.1/B2), subsampled for speed
@torch.no_grad()
def fast_eval(mm,test_data,xs,xm,mode,tok,device,max_tok=128,n_eval=200):
    import random;random.seed(42)
    indices=random.sample(range(len(test_data)),min(n_eval,len(test_data)))
...
if __name__=='__main__':
    print("="*60);print("A: merged_all + Adapter 8t × 3 seeds (fast eval)");print("="*60)
    ...
    for seed in SEEDS:
        ...
        for ep in range(NE):
            ...
            for si,batch in enumerate(dl):
                ...
                if (si+1)%GA==0:torch.nn.utils.clip_grad_norm_(mm.parameters(),1.0);opt.step();opt.zero_grad()
            print(f'  Epoch {ep+1}/{NE} loss={ep_loss/max(n,1):.4f} time={time.time()-t1:.0f}s')
        mm.set_dropout_prob(0.0);enc_nl=fast_eval(mm,ed,xs,xm,'normal',tok,DEVICE)
        mm.set_dropout_prob(1.0);nl_only=fast_eval(mm,ed,xs,xm,'dropout',tok,DEVICE)
        ...
        results.append({'seed':seed,'enc_nl':enc_nl,'nl_only':nl_only,'gap':gap})
        del mm;gc.collect();torch.cuda.empty_cache()
    accs=[r['enc_nl']['accuracy'] for r in results]
    print(f"\nMEAN: {np.mean(accs):.4f} ±{np.std(accs):.4f}")
    with open(f'{RESULT_DIR}/merged_all_3seed.json','w') as f:json.dump(results,f,indent=2,default=str)
    print("DONE")
