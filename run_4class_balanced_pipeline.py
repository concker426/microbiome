#!/usr/bin/env python3
"""
四分类流水线：Healthy / IBD / CD / UC（过采样平衡）
- 对 Healthy、CD、UC 过采样至与 IBD 数量一致
...
OUTPUT_DIR = "/hd/liujx/microbiome_llm_project/saved_models/qwen2_0.5b_4class_balanced"
EVAL_DIR = "/hd/liujx/microbiome_llm_project/eval_results_0.5b_4class"
...
def balance_classes(data):
    ...
    for label, samples in grouped.items():
        if len(samples) == max_count:
            ...
            print(f"  {label}: {len(samples)} → {len(oversampled)} (x{times + remainder/len(samples):.1f})")
...
def main():
    print("=" * 60)
    print("  四分类流水线: Healthy / IBD / CD / UC（过采样平衡）")
    ...
    results_before = evaluate(base_model, tokenizer, raw_test, device,
                              name="基座模型 (零样本, 四分类)", max_new_tokens=64)
    ...
    results_after = evaluate(eval_model, tokenizer, raw_test, device,
                             name="微调后模型 (四分类, 平衡)", max_new_tokens=64)
    ...
    results = {
        ...
        'task': '4class_balanced',
        'train_before': dict(Counter(d['label'] for d in raw_train)),
        'train_after': dict(Counter(d['label'] for d in train_data)),
        'test': dict(Counter(d['label'] for d in raw_test)),
        'training_loss': float(final_loss),
        'results_before': {'accuracy': acc_before, 'macro_f1': f1_before, 'weighted_f1': wf1_before},
        'results_after': {'accuracy': acc_after, 'macro_f1': f1_after, 'weighted_f1': wf1_after},
    }
