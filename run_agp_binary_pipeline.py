#!/usr/bin/env python3
"""
AGP genus-level 数据 + Qwen2-0.5B-Instruct + LoRA 二分类
"""
...
OUTPUT_DIR = "/hd/liujx/microbiome_llm_project/saved_models/qwen2_0.5b_agp_genus"
EVAL_DIR = "/hd/liujx/microbiome_llm_project/eval_results_0.5b_agp"
...
def main():
    print("=" * 60)
    print("  AGP Genus-level 分类")
    print("  Qwen2-0.5B-Instruct + LoRA")
    print("  Healthy vs Disease (过采样平衡)")
    print("=" * 60)
    ...
    print(f"  训练集: {len(train_data)} 样本, 分布: {dict(Counter(d['label'] for d in train_data))}")
    ...
    results_before = evaluate(base_model, tokenizer, test_data, device,
                              name="基座模型 (零样本, AGP genus)", max_new_tokens=64)
    ...
    results_after = evaluate(eval_model, tokenizer, test_data, device,
                             name="微调后 (AGP genus, 平衡)", max_new_tokens=64)
